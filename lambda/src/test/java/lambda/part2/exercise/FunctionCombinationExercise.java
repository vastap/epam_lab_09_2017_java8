...
//        throw new UnsupportedOperationException();
    ...
    public void personHasNotEmptyLastNameAndFirstName3() {
        ...
        Predicate<Person> validateFirstName = (p) -> negate(hasEmptyFirstName).test(p); // TODO use Predicate->negate
        Predicate<Person> validateLastName = (p) -> negate(hasEmptyLastName).test(p); // TODO use Predicate->negate

        Predicate<Person> validate = (p)-> and(validateFirstName,validateLastName).test(p); // TODO use Predicate->and
